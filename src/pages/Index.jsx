...
async function home(searchValue){
  ...
  const dataHome = await fetch(`${import.meta.env.VITE_API_URL}/events/${eventSearch}` ,{
    headers: {
...
useEffect(() =>{
  async function partnersData(){
    const eventfetch = await fetch(`${import.meta.env.VITE_API_URL}/partners/`)
...
async function eventCategory() {
  const Categoryfetch = await fetch(`${import.meta.env.VITE_API_URL}/categories/events/?id=${datacategory}&page=${page}&limit=3`)
