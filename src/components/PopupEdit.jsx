...
function PopupEdit({props, popedit, eventId}) {
 ...
 async function insertEvent(e) {
  ...
  const eventfetch = await fetch(`${import.meta.env.VITE_API_URL}/events/` + eventId, {
    method: 'PATCH',
    headers: {
  ...
  if (listevent.success === true){
      ...
      const categoryResponse = await fetch(`${import.meta.env.VITE_API_URL}/categories/` + eventId, {
        method: 'PATCH',
        body: categoryForm,
      })
